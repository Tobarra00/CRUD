...
def open_db():
    ...
    '''
    db_name = filedialog.askopenfilename(title="Database Search", filetypes=(("", "*.db"),("Any file", "*.*")))
    db_connexion = sqlite3.connect(db_name)
    db_cursor = db_connexion.cursor()
    db_cursor.execute("INSERT INTO CAMPINFO VALUES (NULL, 'PEDRO', 'GARCIA', '24/08/1998', 'NINGUN COMENTARIO')")
    db_connexion.commit()
    db_connexion.close()
